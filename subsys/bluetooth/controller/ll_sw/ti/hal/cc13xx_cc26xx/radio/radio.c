...
static inline bool ble_commandNo_automatically_performs_rx(uint16_t commandNo)
{
...
static const char *const command_no_to_string(uint16_t command_no)
{
...
static void describe_ble_status(uint16_t status)
{
	switch (status) {
	case IDLE:
...
static void ble_cc13xx_cc26xx_data_init(void)
{
	...
	mac = (uint8_t *)drv_data->device_address;
	...
	drv_data->tx_entry[0].pNextEntry = (uint8_t *)&drv_data->tx_entry[1];
	...
	drv_data->tx_entry[1].pNextEntry = (uint8_t *)&drv_data->tx_entry[0];
	...
	drv_data->tx_queue.pCurrEntry = (uint8_t *)&drv_data->tx_entry[0];
...
{
	drv_data->access_address = *(uint32_t *)aa;
}
	...
	if (pdu_is_adv) {
		...
		case PDU_DATA_LLID_CTRL:
			...
			case PDU_DATA_LLCTRL_TYPE_PER_INIT_FEAT_XCHG:
				typespecstr = "DATA: CTRL: PER_INIT_FEAT_XCHG";
				break;
...
void radio_disable(void)
{
	...
	RFCDoorbellSendTo((uint32_t)&drv_data->cmd_clear_rx);
			...
			if (!once) {
				...
				if ( pdu_is_adv ) {
					...
					if ( PDU_ADV_TYPE_CONNECT_IND == pdu_rx->type ) {
						...
						uint32_t transmitWindowStart =
							0
							+ isr_tmr_end
							+ transmitWindowDelay
							+ transmitWindowOffset
							;
						uint32_t transmitWindowEnd =
							0
							+ transmitWindowStart
							+ transmitWindowSize
							;
...
static void update_adv_data(uint8_t *data, uint8_t len, bool scan_rsp)
{
	...
	RFCDoorbellSendTo((uint32_t)&drv_data->cmd_ble_adv_payload);
}

void radio_set_scan_rsp_data(uint8_t *data, uint8_t len)
{
...
static void transmit_window_callback(RF_Handle h, RF_RatHandle rh, RF_EventMask e, uint32_t compareCaptureTime) {
	...
	*/
	...
	uint32_t begin = drv_data->window_begin_ticks;
	uint32_t duration = drv_data->window_duration_ticks;
	uint32_t interval = drv_data->window_interval_ticks;
